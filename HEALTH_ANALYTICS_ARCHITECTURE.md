...
Der synthetische Exportgenerator ist ein getrenntes Entwicklungsmodul mit `generate_export(scenario_id, seed, destination) -> GeneratedFixture`. Entwicklungs-CLI und Tests verwenden dieses Interface; das reale Anwendungsmodul und der reale Datenmodus können es nicht erreichen. Signalform, Rauschen, Missingness und Qualitätsfälle bleiben in versionierten Szenariodefinitionen verborgen. V0.1 liefert mindestens `lag-signal-v1` und `null-v1`; Änderungen erzeugen neue Szenario-IDs.
